...
func CreateSlug(input string) string {
	...
	if err != nil {
		logger.Error("Error creating slug", err)
		return ""
	}
...
func GenerateRandomString(length int, charSet string) string {
	...
	for i := range result {
		...
		if err != nil {
			logger.Error("Failed to generate secure random index", err)
			return ""
		}
...
func GenerateBIP39ReadableCode(wordCount int) string {
	...
	for i := 0; i < wordCount; i++ {
		...
		if err != nil {
			logger.Error("Failed to generate secure index for BIP39 words", err)
			return ""
		}
...
func GenerateBIP39CodeWithHashSuffix(wordCount int) string {
	...
	if err != nil {
		logger.Error("Failed to generate secure suffix", err)
